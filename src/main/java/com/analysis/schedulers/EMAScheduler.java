...
public class EMAScheduler {
	...
	public EMAScheduler(ScripCache scripCache, FivePaisaApiClient executor, EMAService emaService,
		...
		if (!StringUtils.hasText(accessToken)) {
			...
			for (Map.Entry<Integer, String> entry : scripCache.getAllScripEntries()) {
				...
				try {
					...
					log.warn("Rate limit hit for {} ({}). Sleeping for 3 seconds.", symbol, scripCode);
					...
					log.error("Failed processing EMA for {} ({})", symbol, scripCode, ex);
				}
			}
